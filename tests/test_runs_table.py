...
def test_lots_view_aggregates_min_max_and_latest_job(tmp_path):
    ...
    rows = conn.execute(
        ...
        "job_mixed, job_variant_count FROM lots WHERE lot_id = 'LOT1'"
    ).fetchall()
    assert len(rows) == 1   # 1 lot = 1 row
    lot_id, start_time, finish_time, job_name, job_rev, job_mixed, job_variant_count = rows[0]
    ...
    # Two job_revs across the lot's runs -> flagged as TP-mixed.
