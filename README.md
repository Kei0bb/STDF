...
FastAPI + Alpine.js + Plotly.js によるダークテーマ SPA。

**機能:**
- **Dashboard** — ロット別歩留まりバーチャート、KPIカード（総部品数 / 良品数 / 平均歩留まり）
- **Wafer** — ウェハー別歩留まり横棒チャート + ウェハーマップ（Pass/Fail or ソフト/ハードBin 色分け）
- **Tests** — AG Grid でテストパラメーター選択（検索・一括選択）+ 分布ヒストグラム + フェール Pareto チャート
- **Export** — CSV ダウンロード（Pivot形式: 部品×テスト / Long形式: 統計ツール向け）、ロット・ウェハー・テスト番号フィルター対応
