...
if not products and not test_categories:
    ...
    st.code("stdf ingest <file.stdf> --product ABC -s CP11")
    st.stop()
...
st.caption(f"stdf DB v{__version__} | Product → Lot → Wafer (CP only) → Parameters → Export")
